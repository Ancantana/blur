...
function setup() {
  ...
  cursorGlow = createGraphics(width, height);
...
function draw() {
  ...
  // Draw the blurry cursor glow
  cursorGlow.background(0, 0);
  cursorGlow.fill(217, 217, 217, 50);
  cursorGlow.noStroke();
  cursorGlow.ellipse(mouseX, mouseY, cursorGlowSize);
  cursorGlow.filter(BLUR, 10);
  image(cursorGlow, 0, 0);
}
...
function windowResized() {
  ...
  cursorGlow = createGraphics(width, height); // Recreate the graphics buffer
  slider.position(width / 2 - 100, height / 2 + 20);
  moveOnButton.position(width / 2 - moveOnButton.width / 2, height / 2 + 100);
  textInput.position(width / 2 - textInput.width / 2, height / 2 + 28);
